...
| --- | --- | --- | --- |
| `agent-architectures` | Decomposition of agent systems: planning, delegation, multi-agent structure, control loops. | — | A real multi-agent postmortem; something on when delegation stops paying. |
| `context-memory` | What the agent knows and where it keeps it: context windows, compaction, persistent stores, retrieval strategy. | [[llm-wiki-pattern]] | Compaction strategies under long sessions; how memory is evicted. |
...
| `evaluation` | Measuring agent systems: benchmarks, LLM-as-judge, trajectory scoring, regression testing. | — | A trajectory-scoring method with a real failure analysis. |
...
| `safety-sandboxing` | Permissioning, isolation, prompt injection, containment of side effects. | — | A concrete prompt-injection case study against a tool-using agent. |
